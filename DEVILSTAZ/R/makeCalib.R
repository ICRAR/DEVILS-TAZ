...
makeCalib<-function(toReduce=toReduce, doCalibQC=FALSE, logName=logName, verbose=verbose){
    ...
    for (i in 1:length(toReduce)){
        ...
        if (length(list.files(path=biasDir, pattern=paste(biasDate,'_blue_BIAScombined.fits',sep='')))==0){
            ...
            system(paste('aaorun combine_image ',tmp, ' -idxfile data/idxFiles/gama_blue.idx -SKYCOMBINE MEDIAN -OPERATION MEDIAN -COMBINEDFILE ',biasDir,'/',biasDate,'_blue_BIAScombined.fits', sep='"'))
            ...
            system(paste('aaorun combine_image ',tmp, '  -idxfile data/idxFiles/gama_red.idx -SKYCOMBINE MEDIAN -OPERATION MEDIAN -COMBINEDFILE ',biasDir,'/',biasDate,'_red_BIAScombined.fits', sep='"'))
        ...
        biasFileRed<-paste(biasDir,'/',biasDate,'_red_BIAScombined.fits', sep='')
         ...
         if (length(list.files(path=darkDir, pattern='*DARKcombined.fits'))==0){
            ...
            darkfilesRed<-paste(darkDir,'/',darkfiles[which(substr(darkfiles, 6,6)=='2')],sep='')
             ...
             system(paste('aaorun combine_image ',tmp, '  -idxfile data/idxFiles/gama_blue.idx -SKYCOMBINE MEDIAN -OPERATION MEDIAN -COMBINEDFILE ',darkDir,'/',darkDate,'_blue_DARKcombined.fits', sep='"'))

             tmp<-list.files(path=darkDir, pattern='*red.fits')
             tmp<-tmp[which(substr(tmp, 6,6)=='2')]
             tmp<-paste(darkDir,'/',tmp, sep='', collapse=' ')
             write(paste('              - Making Master Dark:',darkDir,'/',darkDate,'_red_DARKcombined.fits', sep=''), file=logName, append=T)
             system(paste('aaorun combine_image ',tmp, ' -idxfile data/idxFiles/gama_red.idx -SKYCOMBINE MEDIAN -OPERATION MEDIAN -COMBINEDFILE ',darkDir,'/',darkDate,'_red_DARKcombined.fits', sep='"'))
